...
pk_str = ', '.join([f'{p:.1f}' for p in pk])
print(f'峰值: {pk_str} Hz')

# 非线性拉伸x轴 (带宽按峰值频率自适应)
bw = [0.012 * p for p in pk]     # ±1.2%峰值 = ±4×半功率带宽
w1 = 15.0 * np.exp(-0.5 * ((f_np - pk[0]) / (bw[0] * 0.5))**2)
w2 =  4.0  # obsolete for 3-mode, kept * np.exp(-0.5 * ((f_np - pk[1]) / (bw[1] * 0.5))**2)
tw = 1.0 + w1 + w2
xs = np.zeros_like(f_np)
for i in range(1, len(f_np)):
    xs[i] = xs[i-1] + (tw[i] + tw[i-1]) / 2 * (f_np[i] - f_np[i-1])

tick_freqs = np.unique(np.sort([F_MIN, pk[0]-bw[0], pk[0], pk[0]+bw[0], pk[1]-bw[1], pk[1], pk[1]+bw[1], F_MAX]))
...
for row, (ax, idx, label) in enumerate(zip(axes, target_indices, labels)):
    ...
    s1 = [np.interp(pk[0]-bw[0], f_np, xs), np.interp(pk[0]+bw[0], f_np, xs)]
    s2 = [np.interp(pk[1]-bw[1], f_np, xs), np.interp(pk[1]+bw[1], f_np, xs)]
...
for ax, (lo, hi), pname in zip(axes2,
    [(pk[0]-bw[0], pk[0]+bw[0]), (pk[1]-bw[1], pk[1]+bw[1])],
    [f'Peak 1 ({pk[0]:.1f} Hz)', f'Peak 2 ({pk[1]:.1f} Hz)']):
