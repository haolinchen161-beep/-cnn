"""
predict.py — 加载 checkpoint 对测试样本预测 FRF (2.5D CNN).
用法: F:\pytorch_cuda12\python.exe sample\predict.py
"""
import os, sys
...
def main():
    ...
    for idx in range(len(testset)):
        ...
        with torch.no_grad():
            if phi_exc_t is not None:
                _, _, _, phi_scan = net(img, coords, freqs, None, None)
                modal_phi_i = sn['modal_phi'].to(device)
                dot = torch.sum(phi_scan * modal_phi_i, dim=0)  # [K]
                phi_exc_t = phi_exc_t * torch.sign(dot + 1e-8).unsqueeze(0)
            frf_p, _, _, _ = net(img, coords, freqs, phi_exc_t, None)
    ...
    print(f"Test amplitude MSE: {np.mean(mse_vals):.6f}")
