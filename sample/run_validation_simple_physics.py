"""MeshGraphNet simplified-physics modal training entrypoint.

This is a separate training channel for ansys/data_simple_physics.
It does not replace sample/run_validation.py.

Use this channel to verify the first-layer simplified task:
    fixed material + fixed boundary + geometry variation -> omega/zeta/phi.

Recommended command:
    F:/pytorch_cuda12/python.exe -B sample/run_validation_simple_physics.py
"""
...
CONFIG = {
    # First-layer validation should be short and modal-focused.
    "epochs": 160,
    "validation_frequency": 5,
    "omega_pretrain_epochs": 25,
    "omega_prior_only_epochs": 5,
    # Disable FRF fine-tuning in this first-layer channel. Validation still reports FRF.
...
class SimpleArgs:
    def __init__(self):
        ...
        self.fp16 = True
        self.dir = os.path.join(os.path.dirname(__file__), "output_meshgraphnet_simple_physics")
        self.debug = False


def main():
    print("=" * 80)
    print("FEM-aware MeshGraphNet | simplified-physics modal validation")
    print("=" * 80)
    args = SimpleArgs()
    data_dir = os.path.join(os.path.dirname(__file__), "..", "ansys", "data_simple_physics")
    print(f"Device: {args.device}, batch_size={args.batch_size}, fp16={args.fp16}")
    print(f"Data dir: {data_dir}")
    print(f"Output dir: {args.dir}")
    print("Training target: modal-only first-layer validation; FRF is reported for diagnosis only.")
