...
本版采用受控随机数据集：装夹刚度 weak/medium/strong 三档 + 小扰动；coverage_level × layout_type × clamp_level 分层，保留 5/6/7 凹槽布局和边界扰动。
默认加入一个简单近频过滤：仅过滤训练目标阶数内的相邻模态，避免第一版标签过于复杂。
...
N_SAMPLES = int(os.getenv("N_SAMPLES", "135"))
N_TRAIN = int(os.getenv("N_TRAIN", "81"))
N_VAL = int(os.getenv("N_VAL", "27"))
N_TEST = int(os.getenv("N_TEST", "27"))
assert N_TRAIN + N_VAL + N_TEST == N_SAMPLES, "N_TRAIN + N_VAL + N_TEST must equal N_SAMPLES"

N_MODES = int(os.getenv("N_MODES", "10"))
# 第一版训练建议只使用前三阶；求解与保存仍保留 N_MODES 阶，便于后续扩展。
TRAIN_TARGET_MODES = int(os.getenv("TRAIN_TARGET_MODES", "3"))
...
# 简单近频过滤：第一版只过滤训练目标阶数内的极端近频模态，避免前三阶标签过于复杂。
# 设置 MIN_RELATIVE_MODE_GAP=0 或 FILTER_CLOSE_TARGET_MODES=0 可关闭过滤。默认 0.01，即 1%。
MIN_RELATIVE_MODE_GAP = float(os.getenv("MIN_RELATIVE_MODE_GAP", "0.01"))
FILTER_CLOSE_TARGET_MODES = os.getenv("FILTER_CLOSE_TARGET_MODES", "1").strip().lower() in {"1", "true", "yes", "y"}

# 推荐：质量归一化 + 一致质量矩阵。
USE_MASS_NORMALIZATION = True
USE_LUMPED_MASS = False

OUT_DIR = os.getenv("OUT_DIR", os.path.join(os.path.dirname(__file__), "data_modal_residue_stage1"))
VIZ_DIR = os.getenv("VIZ_DIR", os.path.join(os.path.dirname(__file__), "mesh_viz_modal_residue_stage1"))
...
GAP_ABS = 0.006
BORDER_ABS = 0.006
TARGET_DEPTH_RANGE = (float(os.getenv("TARGET_DEPTH_MIN", "0.30")), float(os.getenv("TARGET_DEPTH_MAX", "0.60")))
# 第一版深度：每个已加工槽独立采样，但采用三档/四档 + 小扰动，避免连续随机太细导致训练困难。
DEPTH_SAMPLING_MODE = os.getenv("DEPTH_SAMPLING_MODE", "layered")  # layered / continuous
DEPTH_LEVELS = np.asarray(
    [float(x) for x in os.getenv("DEPTH_LEVELS", "0.30,0.45,0.60").split(",") if x.strip()],
    dtype=np.float32,
)
DEPTH_LEVEL_JITTER = float(os.getenv("DEPTH_LEVEL_JITTER", "0.03"))
# 当前第一版不考虑切削过程；后续若要生成铣削瞬间状态，可设 INCLUDE_MACHINING_PROGRESS=1。
INCLUDE_MACHINING_PROGRESS = os.getenv("INCLUDE_MACHINING_PROGRESS", "0").strip().lower() in {"1", "true", "yes", "y"}
...
def build_sample_plan(n_train, n_val, n_test):
    ...
    第一版主分层：coverage_level × layout_type × clamp_level = 3×3×3。
    每个 split 内按相同权重分配，保证 train/val/test 都覆盖加工数量、布局和装夹刚度三档。
    """
    plan = []
    split_specs = [("train", int(n_train)), ("val", int(n_val)), ("test", int(n_test))]
    coverage_labels = ["low", "medium", "high"]
    layout_labels = [5, 6, 7]
    combo_labels = [(g, l, c) for g in coverage_labels for l in layout_labels for c in CLAMP_LEVELS]
    combo_weights = {
        (g, l, c): COVERAGE_WEIGHTS[g] * LAYOUT_WEIGHTS[l]
        for g, l, c in combo_labels
    }
    for split, n_split in split_specs:
        combo_counts = _allocate_counts(n_split, combo_labels, combo_weights)
        for (coverage_level, layout_type, clamp_level), n_combo in combo_counts.items():
...
def sample_clamp_parameters(clamp_level):
    """装夹刚度三档 + 每个装夹区独立小扰动。"""
    k_corner_base = float(K_CORNER_LEVEL_BASE[clamp_level])
    k_side_base = float(K_SIDE_LEVEL_BASE[clamp_level])
...
def sample_depth_ratio():
    """
    每个已加工槽独立采样深度。
    第一版默认使用分层深度 + 小扰动，避免连续深度太细导致训练困难。
    """
    if DEPTH_SAMPLING_MODE == "continuous":
        return float(random.uniform(TARGET_DEPTH_RANGE[0], TARGET_DEPTH_RANGE[1]))
    base = float(random.choice(DEPTH_LEVELS))
    depth = base + random.uniform(-DEPTH_LEVEL_JITTER, DEPTH_LEVEL_JITTER)
    return float(np.clip(depth, TARGET_DEPTH_RANGE[0], TARGET_DEPTH_RANGE[1]))


def sample_machining_state(num_cells, coverage_level):
    """
    返回加工完成后的静态状态。
    第一版不考虑切削过程：随机选择 active_count 个槽，每个槽独立深度。
    若后续需要铣削瞬间状态，可设置 INCLUDE_MACHINING_PROGRESS=1。
    """
    ...
    if not INCLUDE_MACHINING_PROGRESS:
        for cell in order[:active_count]:
            depth_by_cell[int(cell)] = sample_depth_ratio()
        active_depths = depth_by_cell[depth_by_cell > 1e-6]
        target_depth_ratio = float(np.mean(active_depths)) if active_depths.size else float(np.nan)
        return {
            "pocket_order": order,
            "active_count": int(active_count),
            "finished_count": int(active_count),
            "current_cell": int(-1),
            "current_progress": float(1.0),
    ...
    for cell in order[:finished_count]:
        depth_by_cell[int(cell)] = sample_depth_ratio()
    depth_by_cell[current_cell] = sample_depth_ratio() * current_progress
    active_depths = depth_by_cell[depth_by_cell > 1e-6]
    target_depth_ratio = float(np.mean(active_depths)) if active_depths.size else float(np.nan)
    return {
        "pocket_order": order,
...
def build_fe_edge_index_from_grid(grid, n_nodes_total):
    """从 FE cell connectivity 提取边；失败时返回 None，由主流程跳过该样本。"""
    edge_set = set()
    try:
        ...
        print(f"  警告: FE拓扑提取失败，将跳过该样本: {exc}")
        return None
...
def build_pocket_features(cell_bounds, cell_depth_ratio):
    """
    输出统一 7 个 pocket token，避免训练时直接处理 NaN。
    feature:
    [x0/L, x1/L, y0/W, y1/W, exists, is_cut, depth_ratio, residual_thickness_ratio]
    """
    feats = np.zeros((7, 8), dtype=np.float32)
    for i in range(7):
        bounds = cell_bounds[i]
        depth = cell_depth_ratio[i]
        if np.any(np.isnan(bounds)):
            feats[i, 4] = 0.0
            continue
        x0, x1, y0, y1 = bounds
        depth_val = 0.0 if np.isnan(depth) else float(depth)
        feats[i, 0] = x0 / L_BASE
        feats[i, 1] = x1 / L_BASE
        feats[i, 2] = y0 / W_BASE
        feats[i, 3] = y1 / W_BASE
        feats[i, 4] = 1.0
        feats[i, 5] = 1.0 if depth_val > 1e-6 else 0.0
        feats[i, 6] = depth_val
        feats[i, 7] = 1.0 - depth_val
    return feats


def build_clamp_features(all_clamp_areas, K_corners, C_corners, K_sides, C_sides):
    """
    输出 7 个装夹 token，便于第一版参数化/坐标查询网络直接使用。
    同时仍保留 spring_k_xyz、spring_c_xyz、node_type，支持后续 GNN。
    feature:
    [x0/L, x1/L, y0/W, y1/W, type, Kx, Ky, Kz, Cx, Cy, Cz]
    type: 1=corner, 0.5=side
    """
    feats = np.zeros((7, 11), dtype=np.float32)
    for i, (xmin, xmax, ymin, ymax) in enumerate(all_clamp_areas):
        is_corner = i < 4
        if is_corner:
            K = float(K_corners[i])
            C = float(C_corners[i])
        ...
        feats[i] = np.asarray([
            xmin / L_BASE, xmax / L_BASE, ymin / W_BASE, ymax / W_BASE, ctype,
            np.log10(Kx + 1.0), np.log10(Ky + 1.0), np.log10(Kz + 1.0),
            np.log10(Cx + 1.0), np.log10(Cy + 1.0), np.log10(Cz + 1.0),
        ], dtype=np.float32)
    return feats
...
def save_h5(name, idx_slice, arrays):
    ...
    with h5py.File(path, "w") as f:
        ...
        f.attrs["train_target_modes"] = TRAIN_TARGET_MODES
        ...
        f.attrs["filter_close_target_modes"] = int(FILTER_CLOSE_TARGET_MODES)
        f.attrs["freq_grid_min_step_hz"] = FREQ_GRID_MIN_STEP_HZ
        f.attrs["sampling_strategy"] = "stratified coverage_level x layout_type x clamp_level; independent layered pocket depths; static finished state by default"
        ...
        for i, idx in enumerate(idxs):
            ...
            for key, values in arrays.items():
                compression = "gzip" if key in {
                    "edge_index", "edge_attr", "point_frf", "modal_phi_z", "modal_phi_xyz", "modal_residue_z", "spring_k_xyz", "spring_c_xyz",
                    "node_type", "pocket_bottom_mask", "cut_region_mask",
                    "local_thickness_ratio", "pocket_depth_ratio", "pocket_features", "clamp_features",
                } else None
...
print(f"配置: {N_SAMPLES}样本, train/val/test={N_TRAIN}/{N_VAL}/{N_TEST}, {N_MODES}阶模态, 训练目标{TRAIN_TARGET_MODES}阶, {N_FREQS}个频率点")
print(f"模态归一化: mass, 质量矩阵: {'lumped' if USE_LUMPED_MASS else 'consistent'}")
if MIN_RELATIVE_MODE_GAP > 0:
    print(f"样本过滤: 训练目标阶数内近频过滤 min_relative_gap >= {MIN_RELATIVE_MODE_GAP:.3f}；另过滤频率非递增、激励点无效、ANSYS求解失败等异常样本")
else:
...
print("采样: 装夹刚度三档+小扰动；coverage_level×layout_type×clamp_level 分层；每个槽独立分层深度；边界 jitter 受控随机")
...
csv_header = [
    ...
    "clamp_model", "clamp_level", "coverage_level", "layout_type", "n_cols", "n_rows",
...
while valid_samples < N_SAMPLES:
    ...
    try:
        ...
        (
            ...
            K_corner_base, K_side_base, clamp_level_code,
        ...
        pocket_features = build_pocket_features(cell_bounds, cell_depth_ratio)
        ...
        try:
            ...
            edge_index = None
        if edge_index is None or edge_index.size == 0 or edge_index.shape[1] == 0:
            print("  跳过: FE edge_index 提取失败，不使用 kNN 兜底")
            continue
        ...
        clamp_features = build_clamp_features(all_clamp_areas, K_corners, C_corners, K_sides, C_sides)
        ...
        target_freq_hz = freq_hz[:TRAIN_TARGET_MODES]
        target_mode_gaps_hz = np.diff(target_freq_hz.astype(np.float64))
        if len(target_mode_gaps_hz) > 0:
            target_relative_gaps = target_mode_gaps_hz / np.maximum(target_freq_hz[:-1].astype(np.float64), 1e-12)
            target_min_relative_gap = float(np.min(target_relative_gaps))
            target_min_relative_gap_pair = int(np.argmin(target_relative_gaps) + 1)
        else:
            target_min_relative_gap = np.inf
            target_min_relative_gap_pair = -1

        if FILTER_CLOSE_TARGET_MODES and MIN_RELATIVE_MODE_GAP > 0.0 and target_min_relative_gap < MIN_RELATIVE_MODE_GAP:
            skip_close_mode_count += 1
            print(
                f"  跳过: 前{TRAIN_TARGET_MODES}阶近频 pair {target_min_relative_gap_pair}-{target_min_relative_gap_pair + 1}, "
                f"target_min_rel_gap={target_min_relative_gap:.4f} < {MIN_RELATIVE_MODE_GAP:.4f}"
            )
            continue
        ...
        arrays["modal_phi_z"].append(phi_z.astype(np.float32))
        ...
        arrays["pocket_features"].append(pocket_features)
        arrays["clamp_features"].append(clamp_features)
        ...
        arrays["clamp_model_code"].append(np.array(clamp_level_code, dtype=np.int64))
        arrays["clamp_level_code"].append(np.array(clamp_level_code, dtype=np.int64))
