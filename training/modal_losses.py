...
def phi_loss(pred, target, batch, weight=None):
    if weight is None:
    ...
    for i in range(n):
        ...
        scale = torch.std(t.transpose(0, 1).reshape(t.shape[1], -1), dim=1).clamp_min(1e-8)
        mses.append(torch.mean(w * ((p - t) / scale.view(1, -1, 1)) ** 2))
        macs.append(mac(p, t))
    return torch.stack(mses).mean(), torch.stack(macs, dim=0)


def modal_loss(out, batch, freq_weight=1.0, phi_weight=1.0, mac_weight=5.0):
    lf, fm = frequency_loss(out["omega"], batch["modal_omega_phys"])
    target = align_phi(out["phi"], batch["modal_phi"], batch["batch"])
    lmse, macs = phi_loss(out["phi"], target, batch["batch"], batch.get("node_weight"))
    lphi = lmse + mac_weight * (1.0 - macs.mean())
    loss = freq_weight * lf + phi_weight * lphi
    return loss, {
        "loss": loss.detach(), "loss_freq": lf.detach(), "loss_phi": lphi.detach(),
        "phi_mse": lmse.detach(), "phi_mac": macs.mean().detach(),
        "phi_mac_mode1": macs[:, 0].mean().detach(),
        "phi_mac_mode2": macs[:, 1].mean().detach(),
        "phi_mac_mode3": macs[:, 2].mean().detach(),
        **fm,
    }
