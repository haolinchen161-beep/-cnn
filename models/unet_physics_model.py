...
class UNetPhysicsModel(nn.Module):
    ...
    def forward(self, image_tensor, query_coords, frequencies=None,
        ...
        if query_coords.ndim == 3:
            ...
            batch = batch if batch is not None else torch.zeros(query_coords.shape[0], dtype=torch.long, device=image_tensor.device)
        ...
        for b in range(B):
            mask = batch == b
            coords_b = coords_flat[mask].unsqueeze(0).unsqueeze(0)  # [1,1,N_b,2]
